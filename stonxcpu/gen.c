/* 
 * gen.c - generates the m4 code from 68000.def
 *
...
void gobble (void)
{
        ...
        do {
          c = getchar();
          if(c == '#') {
            while(c != 10 && c != 13 && c != EOF) c = getchar();
          }
          if(c == EOF) return;
        } while (isspace(c));
        ungetc(c,stdin);
...
main(void)
{
...
#else
#ifdef HALF_TABLES
        printf("GENFUNC_PROTO((*jumptab1[32768]))={\n");
        for (i=0; i<65536; i++)
        {
                if (tab[i])
                {
        ...
        printf("\n};\n");
        printf("#else /* FIRST_TABLE */\n");
        printf("GENFUNC_PROTO((*jumptab2[32768]))={\n");
...
#if 0   
        f = fopen("mapping.txt", "w");
        fprintf(f, "mapping between func number and func purpose\n\n");
        { 
          int j = -1;
        ...
        fprintf(f, "#define NUMBER_OF_FUNCS %d\n\n", nfunc);
        fprintf(f, "GENFUNC_PROTO((*funclist[%d]))={\n", nfunc);
        for(i = 0 ; i<nfunc; i++)
        {
                fprintf(f, "Func%d,",i);
                if((i & 7) == 7) 
