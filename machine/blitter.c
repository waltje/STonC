...
#include "iofuncs.h"
#include "mem.h"
#include "options.h"    /* for BLITTER */

#if (0)
{        ec_stderr("Source Address:%X\n",source_addr);             \
        ec_stderr("  Dest Address:%X\n",dest_addr);                \
        ec_stderr("       X count:%X\n",x_count);                  \
        ec_stderr("       Y count:%X\n",y_count);                  \
        ec_stderr("  Source X inc:%X\n",source_x_inc);             \
        ec_stderr("    Dest X inc:%X\n",dest_x_inc);               \
        ec_stderr("  Source Y inc:%X\n",source_y_inc);             \
        ec_stderr("    Dest Y inc:%X\n",dest_y_inc);               \
        ec_stderr("HOP:%2X    OP:%X\n",hop,op);                    \
        ec_stderr("   source SKEW:%X\n",skewreg);                  \
        ec_stderr("     endmask 1:%X\n",end_mask_1);               \
        ec_stderr("     endmask 2:%X\n",end_mask_2);               \
        ec_stderr("     endmask 3:%X\n",end_mask_3);               \
        ec_stderr("       linenum:%X\n",line_num);                 \
        if (NFSR) ec_stderr("NFSR is Set!\n");                     \
        if (FXSR) ec_stderr("FXSR is Set!\n");                     \
}
#endif
