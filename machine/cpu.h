...
#define CPU_STATUS_UNINITIALIZED 0
#define CPU_STATUS_STOPPED 1   /* emulation sort of halted, executing stop */
#define CPU_STATUS_HALTED 2         /* emulation halted, but the CPU is alive */
#define CPU_STATUS_RUNNING 3
#define CPU_STATUS_DEAD 4     /* 'halted' condition, after a double bus error */

extern void set_cpu_status(int);
...
/* in cpubis.c : */
extern void ex_reset(void);
extern void ex_stop(void);
...
extern void enter_supervisor_mode(void);
...
#else
#if 0  /* use old C method with recursion (nasty)*/
...
#else
#define throw(_n) longjmp(cpu_jmp_buf, _n)
#define BUS_ERROR(_a,_rw)  do{ ex_addr=_a; ex_rw=_rw; throw(2); }while(0)
#define ADDRESS_ERROR(_a,_rw)  do{ ex_addr=_a; ex_rw=_rw; throw(3); }while(0)
#define ILLEGAL_INSTRUCTION throw(4) 
#endif
